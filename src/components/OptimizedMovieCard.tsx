...
export const OptimizedMovieCard = ({ movie, priority = false }: OptimizedMovieCardProps) => {
      ...
      <div className="bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-105 cursor-pointer">
        <div className="relative">
          {!imageLoaded && !imageError && (
            <div className="w-full h-48 sm:h-64 bg-gray-700 animate-pulse flex items-center justify-center">
              <div className="w-8 h-8 border-2 border-gray-500 border-t-transparent rounded-full animate-spin"></div>
            </div>
          )}
          
          <img 
            src={imageError ? "https://images.unsplash.com/photo-1518676590629-3dcbd9c5a5c9?w=500" : movie.image}
            alt={movie.title}
            className={`w-full h-48 sm:h-64 object-cover transition-opacity duration-300 ${
